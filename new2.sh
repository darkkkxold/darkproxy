...
function create_startup_script() {
  ...
  rnd_subnet_ip() {
    local base_subnet=$(get_subnet_mask | cut -d'/' -f1)  # Удаляем маску подсети
    local hex_groups=({0..9} {a..f})
    local result="$base_subnet"
    
    # Добавляем 4 случайные 16-битные группы
    for i in {1..4}; do
        result+=":"
        for j in {1..4}; do
            result+=${hex_groups[$RANDOM % 16]}
        done
    done
